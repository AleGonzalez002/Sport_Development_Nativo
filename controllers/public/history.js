...
const HISTORIAL_API = 'services/public/history.php';
...
ITEM_FORM.addEventListener('submit', async (event) => {
    ...
    const DATA = await fetchData(HISTORIAL_API, 'updateDetail', FORM);
...
async function deleteOrder(idPedido) {
    ...
    if (RESPONSE) {
        ...
        const DATA = await fetchData(HISTORIAL_API, 'deleteOrder', { id_pedido: idPedido });
        // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
        if (DATA.status) {
